...
* `pres_years`  When to add each block of presence points to the plot. This is a list of integers, and there should be N/2 of them, where N is the number of integers in `presdata_range`. So, 3,5,7 corresponds to "plot the first block of presence locations starting at t=3, the second block at t=5, and the third block at t=7". Note that the initial presence points are always plotted.
